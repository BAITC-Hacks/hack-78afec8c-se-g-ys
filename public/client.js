...
attemptList.addEventListener('click', (event) => { const open = event.target.closest('[data-open-attempt]'); const copy = event.target.closest('[data-copy-attempt]'); if (!open && !copy) return; const attempt = attemptHistory.list()[Number((open ?? copy).dataset.openAttempt ?? (open ?? copy).dataset.copyAttempt)]; if (!attempt) return; if (open) { acceptedDecisions = attempt.decisions; currentAttemptId = attempt.id; currentGlobalComparison = attempt.globalComparison || null; globalComparisonError = null; showResult(attempt.result); } if (copy) copyAttempt(attempt); });
