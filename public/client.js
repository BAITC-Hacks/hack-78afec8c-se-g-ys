...
const globalComparisonVersions = window.qalaGlobalComparisonVersions;
...
let globalComparisonError = null;
...
function globalComparisonMarkup() {
  if (!currentAttemptId && !currentGlobalComparison) return '';
  const button = `<button type="button" data-reveal-global-comparison ${globalComparisonPending || !currentAttemptId ? 'disabled' : ''}>${globalComparisonPending ? text.calculatingGlobalComparison : text.revealGlobalComparison}</button>`;
  if (!currentGlobalComparison) return `<section class="global-comparison"><h3>${text.globalComparison}</h3><p>${text.globalNotice}</p>${globalComparisonError ? `<p role="alert">${escapeHtml(globalComparisonError)}</p>` : ''}${button}</section>`;
  const comparison = currentGlobalComparison;
  const decisions = comparison.global.decisions.map(decisionLabel).map(escapeHtml).join(', ');
  return `<section class="global-comparison"><h3>${text.globalComparison}</h3><div class="result-summary"><span>${text.globalMaximum} <b>${comparison.global.scoreDisplay}</b></span><span>${text.differenceToMaximum} <b>+${comparison.delta.toFixed(5)}</b></span><span>${text.scenarioCost} <b>${comparison.global.cost}</b></span></div><p><b>${text.globalMaximum}:</b> ${decisions}</p><p>${text.globalNotice} ${comparison.global.tieCount > 1 ? `${text.globalTie}: ${comparison.global.tieCount}.` : ''}</p><button type="button" data-copy-global-comparison>${text.copyGlobalScenario}</button>${button}</section>`;
}
...
function showResult(result) {
  ...
  document.querySelector('[data-reveal-global-comparison]')?.addEventListener('click', () => { void requestGlobalComparison(result); });
  document.querySelector('[data-copy-global-comparison]')?.addEventListener('click', copyGlobalScenario);
}
...
async function requestGlobalComparison(result) {
  if (globalComparisonPending || !currentAttemptId) return;
  globalComparisonPending = true;
  globalComparisonError = null;
  showResult(result);
  try {
    const response = await fetch(`/api/attempts/${encodeURIComponent(currentAttemptId)}/what-if`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ locale, ...globalComparisonVersions }),
    });
    const payload = await response.json();
    if (!response.ok) throw new Error(payload.error || text.globalFailed);
    currentGlobalComparison = payload.comparison;
    attemptHistory.saveGlobalComparison(currentAttemptId, currentGlobalComparison);
    renderAttemptHistory();
  } catch {
    globalComparisonError = text.globalFailed;
  } finally {
...
function copyGlobalScenario() {
  ...
  draft.splice(0, draft.length, ...currentGlobalComparison.global.decisions.map((decision) => ({
...
attemptList.addEventListener('click', (event) => {
  ...
  if (!attempt) return;
  if (open) {
    acceptedDecisions = attempt.decisions;
    currentAttemptId = attempt.id;
    currentGlobalComparison = attemptHistory.getGlobalComparison(attempt.id, globalComparisonVersions);
    globalComparisonError = attempt.globalComparison && !currentGlobalComparison ? text.globalVersionMismatch : null;
