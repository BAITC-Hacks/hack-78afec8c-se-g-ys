...
Для запуска полной текущей веб-версии достаточно Docker: образ содержит Node-сервер QALA и не требует установки зависимостей на хосте.

```bash
docker build --tag qala .
docker run --rm --name qala --publish 3000:3000 qala
```

Откройте `http://127.0.0.1:3000`. Чтобы использовать AI-разбор, передайте серверный ключ при запуске, например `docker run --rm --publish 3000:3000 --env OPENAI_API_KEY qala`. Не добавляйте ключ в образ или репозиторий.
