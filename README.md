...
## What If: глобальное сравнение

Только после принятия сценария участник может явно нажать «Сравнить с лучшим допустимым сценарием». Сервер перебирает все 694395 допустимых неупорядоченных сценариев текущего синтетического каталога, применяя тот же валидатор и расчёт Score, что и при принятии сценария. Он показывает максимальный Score, разницу с попыткой, пять решений с целями, направлениями и стоимостью, а также общий расход.

При одинаковом неокруглённом Score представитель выбирается воспроизводимо: меньший расход, затем канонический порядок ID мероприятия и района. Результат сохраняется с попыткой в `localStorage`. `catalogVersion` и `modelVersion` входят в ответ; запрос с несовпадающей версией отклоняется, поэтому результат другого набора данных или формулы не выдаётся за актуальный.

Глобальный максимум относится только к синтетическому каталогу и формуле QALA, а не является рекомендацией политики реального города. Это отдельный инструмент от «Рекомендуемой точечной замены», которая меняет только одно решение. На текущем каталоге холодный перебор в проверке занимает около 8 секунд; в процессе сервера результат кэшируется для той же версии каталога и модели.
