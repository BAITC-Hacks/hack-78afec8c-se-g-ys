...
Для принятой попытки из пяти решений результат содержит доступное на русском и казахском разложение влияния. Сервер вычисляет значения Шепли по всем 32 внутренним подмножествам, применяя тот же детерминированный расчёт, что и для принятого сценария: лаги, синергии, ограничение показателей, критические показатели, самый слабый район и взвешивание населения.

`POST /api/attempts/:attemptId/impact-breakdown` принимает `{ "locale": "ru" | "kk" }`. Он возвращает пять подписанных вкладов, их сверку с разницей полного сценария и базового состояния, версию каталога/модели и понятное объяснение. Внутренние полезности подмножеств намеренно не возвращаются, не становятся пользовательскими Score и не сохраняются как попытки. Успешное разложение сохраняется только с соответствующей попыткой в браузере; несовпадение версии каталога или модели делает сохранённый результат недоступным. Если расчёт временно недоступен, принятой результат остаётся видимым, а пользователь может повторить запрос.
